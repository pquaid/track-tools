...
static double toDouble(const char * val) {
    ...
    if ((end == val) || (end == 0) || (*end != '\0')) {
        throw GPXError("Unable to convert numeric value: " +
                       string(val));
    }
    return d;
}
...
static void processDoc(Document & doc, Track & points) {

    string trackName;

    xml_node<> * trk = doc.getTop().first_node()->first_node("trk");
    if (trk == 0) throw GPXError("No <trk> element");

    xml_node<> * name = trk->first_node("name");
    if (name != 0) {
    ...
    for (xml_node<> * trkseg = trk->first_node();
        ...
        if (strcmp(trkseg->name(), "trkseg") == 0) {
            ...
            for (xml_node<> * trkpt = trkseg->first_node("trkpt");
                ...
                xml_node<> * ts = trkpt->first_node("time");
		if (ts != 0) {
                    struct tm time;
                ...
                xml_node<> * ext = trkpt->first_node("extensions");
                if (ext != 0) {
                    ext = ext->first_node("gpxtpx:TrackPointExtension");
                    if (ext != 0) {
                        xml_node<> * hr = ext->first_node("gpxtpx:hr");
                        if (hr != 0) {
                            current.hr = strtol(hr->value(), 0, 10);
                        }
                        xml_node<> * atemp = ext->first_node("gpxtpx:atemp");
                        if (atemp != 0) {
                ...
                if (!points.empty()) {
                    ...
                    current.length = points[last].length + current.distance(points[last]);
                }
...
void GPX::write(ostream & out, Track & track) {
